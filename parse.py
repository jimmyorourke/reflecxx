...
def find_annotate_attr(cursor: Cursor) -> Optional[Cursor]:
    """Given a cursor, walks its dierct children to determine if there is an annotation attribute. If so returns the
    cursor."""
...
def main(
    ...
    with VisitorGenerator(output_file=output_file, namespace=namespace) as v:
        v.generate_struct_visitors(structures.values())
