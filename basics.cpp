...
struct Acceptor {
    static_assert(dependent_false<EnumType>{}, "Must specialize the Acceptor class for each Enum Type");
...
constexpr void visit(VisitorType&& visitor, T&& t) {
    Acceptor<T, VisitorType>::visit(std::forward<VisitorType>(visitor), std::forward<T>(t));
}
...
constexpr std::underlying_type_t<EnumType> toIndex(EnumType enumVal) {
    ...
    auto l = [enumVal, &index](const EnumType& e, const char* name, std::underlying_type_t<EnumType> val) {
        if (e == enumVal) {
            index = val;
        }
    };
...
constexpr const char* toString(EnumType enumVal) {
    const char* s;
    auto l = [enumVal, &s](const EnumType& e, const char* name, std::underlying_type_t<EnumType> val) {
        if (e == enumVal) {
            s = name;
        }
    };
